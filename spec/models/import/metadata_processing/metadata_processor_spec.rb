...
RSpec.describe Import::MetadataProcessing::MetadataProcessor do
  fixtures :all
  ...
  let(:channel_metadata) do
    Import::Youtube::ChannelMetadata.new(
  ...
  let(:thumbnail_url) do
    Import::Youtube::ThumbnailUrl.new(
  ...
  let(:youtube_metadata) do
    Import::Youtube::VideoMetadata.new(
      slug: "AQ9Ri3kWa_4",
  ...
  let(:music_metadata) do
    Import::MusicRecognition::Metadata.new
  end
  let(:metadata) do
    Import::Metadata.new(
