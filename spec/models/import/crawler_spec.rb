...
RSpec.describe Import::Crawler do
  fixtures :all
  let(:slug) { videos(:video_1_featured).youtube_id }

  describe "#metadata" do
    let(:music_recognizer) do
      music_metadata = Import::MusicRecognition::Metadata.new(acr_id: "a8d9899317fd427b6741b739de8ded15")
      music_recognizer = Import::MusicRecognition::MusicRecognizer.new
      allow(music_recognizer).to receive(:process_audio_snippet).and_return music_metadata
      music_recognizer
    end

    let(:metadata_provider) do
      metadata = Import::Youtube::VideoMetadata.new(slug:)
      metadata_provider = Import::Youtube::MetadataProvider.new
      allow(metadata_provider).to receive(:video_metadata).and_return metadata
      metadata_provider
    end

    it "returns the video data from youtube" do
      video_crawler = Import::Crawler.new(metadata_provider:, music_recognizer:)
    ...
    it "returns the video data from youtube without music recognition" do
      ...
      expect(metadata.youtube.slug).to eq "AQ9Ri3kWa_4"
      expect(metadata.music).to be_a(Import::MusicRecognition::Metadata)
