...
RSpec.describe ExternalVideoImport::VideoCrawl::Youtube::YoutubeScraper do
  fixtures :all
  let(:slug) { videos(:video_1_featured).youtube_id }

  describe "video_metadata" do
    it "returns the video metadata from youtube", vcr: {record: :new_episodes} do
      allow_any_instance_of(ExternalVideoImport::VideoCrawl::Youtube::YoutubeScraper).to receive(:retrieve_html).and_return(File.read("spec/fixtures/files/youtube_video.html"))

      metadata = ExternalVideoImport::VideoCrawl::Youtube::YoutubeScraper.new.video_metadata slug: slug
      ...
      expect(metadata.channel.view_count).to eq 305641
      expect(metadata.channel.video_count).to eq 169
      expect(metadata.channel.subscriber_count).to eq 391
