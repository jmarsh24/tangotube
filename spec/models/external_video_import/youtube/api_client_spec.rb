...
RSpec.describe ExternalVideoImport::Youtube::ApiClient do
  fixtures :all
  ...
  describe "#video_metadata" do
    ...
    it "returns correct video metadata" do
      metadata = ExternalVideoImport::Youtube::ApiClient.new.metadata(slug)
      ...
      expect(metadata.channel.view_count).to eq 305776
      expect(metadata.channel.video_count).to eq 169
      expect(metadata.channel.subscriber_count).to eq 392
