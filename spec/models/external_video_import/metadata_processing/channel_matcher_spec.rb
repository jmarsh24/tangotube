...
RSpec.describe ExternalVideoImport::MetadataProcessing::ChannelMatcher do
  ...
  describe "#match_or_create" do
    context "when the channel exists" do
      let(:channel_metadata) { ExternalVideoImport::Youtube::ChannelMetadata.new(id: existing_channel.youtube_slug, thumbnail_url: "https://yt3.ggpht.com/ytc/AL5GRJVepNM3-zJP50OTXSq6fXyvHB3F9nzST-3sJ2P5=s68-c-k-c0x00ffffff-no-rj") }
      ...
      it "does not call the ThumbnailAttacher" do
        ...
        channel_matcher.match_or_create(channel_metadata:)
      end
    end
    ...
    context "when the channel does not exist" do
      ...
      it "returns the newly created channel" do
        expect(channel_matcher.match_or_create(channel_metadata:)).to eq(new_channel)
      end
