...
RSpec.describe ExternalVideoImport::MusicRecognition::MusicRecognizer do
  ...
  describe "process_audio_snippet" do
    let(:acr_cloud) do
      ...
      allow(acr_cloud).to receive(:analyze).and_return ExternalVideoImport::MusicRecognition::Metadata.new code: 0
      acr_cloud
    end

    let(:audio_trimmer) do
      audio_trimmer = ExternalVideoImport::MusicRecognition::AudioTrimmer.new
      allow(audio_trimmer).to receive(:trim).and_yield trimmed_audio
      audio_trimmer
    end

    let(:youtube_audio_downloader) do
      youtube_audio_downloader = ExternalVideoImport::MusicRecognition::YoutubeAudioDownloader.new
