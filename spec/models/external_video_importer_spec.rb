...
RSpec.describe ExternalVideoImporter do
  fixtures :all
  ...
  describe "#Video" do
    it "creates a new video" do
      ...
      video.update!(event_id: nil)
      video.update!(dancers: [])
      video.destroy!

      expect(Video.find_by(youtube_id: video_metadata.youtube.slug)).not_to be_present
      ExternalVideoImporter::Video.new.call(video_metadata)
      new_video = Video.find_by(youtube_id: video_metadata.youtube.slug)
      expect(new_video).to be_present
      expect(new_video.song).to be_present
      expect(new_video.event).to be_present
      expect(new_video.dancers).to be_present
      expect(new_video.dancers.size).to eq 2
      expect(new_video.couples).to be_present
      expect(new_video.couples.size).to eq 1
    end
  end
  ...
  def video_metadata
    ...
    youtube_video_metadata = YoutubeVideoMetadata.new(
      slug: "AQ9Ri3kWa_4",
