class Video::RelatedVideos
  ...
  def with_same_dancers 
    ...
    Video::Filter.new(videos, filtering_params: {leader:, follower:, hidden: false}).videos
  end

  def with_same_event
    videos = Video.within_week_of(@video.upload_date).exclude_youtube_id(@video.youtube_id)
    event = @video.event.slug
    Video::Filter.new(videos, filtering_params: {event:, hidden: false}).videos
  end

  def with_same_song
    videos = Video.has_leader.has_follower.exclude_youtube_id(@video.youtube_id)
    song = @video.song.slug
    Video::Filter.new(videos, filtering_params: {song:, hidden: false}).videos
  end

  def with_same_channel
    videos = Video.has_leader.has_follower.exclude_youtube_id(@video.youtube_id)
    channel = @video.channel.channel_id
    Video::Filter.new(videos, filtering_params: {channel:, hidden: false}).videos
  end
  ...
  def with_same_performance
    ...
    follower = @video.followers.first.slug
    channel = @video.channel.channel_id
    Video::Filter.new(videos, filtering_params: {channel:, leader:, follower:, hidden: false}).videos
  end
end
