WITH combined_counts AS (
    SELECT
        v.id AS video_id,
        COALESCE(COUNT(likes.likeable_id), 0) AS total_likes,
        COALESCE(COUNT(DISTINCT watches.user_id), 0) AS total_watches,
        COALESCE(COUNT(features.featureable_id), 0) AS total_features,
        CASE
    ...
    FROM
        videos v
        LEFT JOIN likes ON v.id = likes.likeable_id AND likes.likeable_type = 'Video'
        LEFT JOIN watches ON v.id = watches.video_id
        LEFT JOIN features ON v.id = features.featureable_id AND features.featureable_type = 'Video'
    GROUP BY v.id
),
